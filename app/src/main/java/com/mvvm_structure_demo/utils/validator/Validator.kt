...
object Validator {
    ...
    fun isTermChecked(isTermChecked: Boolean): ValidationErrorModel? {
        return if (!isTermChecked)
            ValidationErrorModel(R.string.terms, ValidationError.TERMS)
        else
            null
    }
    ...
    fun validateData(data: String,error : Int): ValidationErrorModel? {
        return if (isBlank(data)) ValidationErrorModel(error, ValidationError.DATA) else null
    }
    ...
    fun validateTelephone(phone: String): ValidationErrorModel? {
        return when {
            ...
            //phone.length !in 10..10 -> ValidationErrorModel(R.string.invalid_phone, ValidationError.PHONE)
            else -> null
        }
    }

    fun validatePincode(pincode: String): ValidationErrorModel? {
        return when {
            isBlank(pincode) -> ValidationErrorModel(R.string.blank_pincode, ValidationError.PHONE)
            else -> null
        }
    }

    fun validateOtp(otp: String): ValidationErrorModel? {
        return when {
            isBlank(otp) -> ValidationErrorModel(R.string.blank_otp, ValidationError.OTP)
            otp.length !in 4..4 -> ValidationErrorModel(R.string.invalid_otp, ValidationError.OTP)
            else -> null
        }
    }
